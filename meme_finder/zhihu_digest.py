...
from .zhihu_seen import SeenStore, filter_unseen
...
TOPVOTED_BRIEF_SYSTEM = """用户在看一条知乎“全站高赞”内容（不限话题，纯按赞数选出）。会给你标题/赞数/摘要。
用简体中文只输出一行，别重复标题、别瞎编：
- **为什么高赞**：一句话点出它戳中了什么（实用/共鸣/反常识/情绪价值/信息量）。
只输出这一行。"""
...
class DigestData:
    ...
    top_voted: List[Pick] = field(default_factory=list)
...
def _rotate(pool: List[str], k: int, seed: int) -> List[str]:
    """从 pool 里取一个长度 k 的滑动窗口（按 seed 起点循环），用于每天轮换种子词。"""
...
def collect_digest_data(
    ...
    *,
    ...
    top_voted_top_n: int = 6,
    top_voted_min_votes: int = 300,
    top_voted_max_age_days: int = 365,
    top_voted_query_count: int = 10,
    ...
    # 二、全站高赞（不限话题）：宽泛种子词撒网、纯按赞数排；每天轮换一批种子词。
    if top_voted_top_n > 0:
        broad_q = _rotate(BROAD_QUERIES, top_voted_query_count, now.timetuple().tm_yday)
        top_voted_ranked = top_voted_recent(
            client,
            broad_q,
            count_per_query=10,
            min_votes=top_voted_min_votes,
            max_age_days=top_voted_max_age_days,
        )
        top_voted_items = filter_unseen(top_voted_ranked, seen_store)[:top_voted_top_n]
        top_voted_notes = _llm_enrich(
            top_voted_items, system_prompt=TOPVOTED_BRIEF_SYSTEM, include_badge=False, api_key=api_key, model=model
        )
        data.top_voted = [Pick(item=it, note=note) for it, note in zip(top_voted_items, top_voted_notes)]
        # 立刻标记，避免同一次运行里“全站高赞”与后面的搞笑/拓展板块重复选到同一条。
        if seen_store is not None:
            for pick in data.top_voted:
                seen_store.mark(pick.item)

    # 二、搞笑精选（纯热度）。先去重、再取 top_n、最后才做 LLM 富集（省 token）。
    funny_ranked = search_fanout(
        client, funny_q, count_per_query=10, min_votes=funny_min_votes, rank_by="engagement"
    )
    funny_items = filter_unseen(funny_ranked, seen_store)[:funny_top_n]
    ...
    # 三、拓展边界（作者含金量主导，每位作者最多 1 条）。同样先去重再富集。
    ...
    knowledge_ranked = _limit_per_author(knowledge_ranked, max_per_author=1)
    knowledge_items = filter_unseen(knowledge_ranked, seen_store)[:knowledge_top_n]
    ...
    # 记录本次实际选入的条目，供下次去重（纯本地、零 token）。
    if seen_store is not None:
        for pick in data.funny:
            seen_store.mark(pick.item)
        for pick in data.knowledge:
            seen_store.mark(pick.item)
        seen_store.prune()
...
def render_markdown(data: DigestData) -> str:
    ...
    lines.append("## 二、全站高赞（不限话题 · 谁赞高谁上）")
    lines.append("")
    lines.append("_排序：纯按赞数，不限话题、不看作者；仅保留较近发布的内容。_\n")
    if not data.top_voted:
        lines.append("_今天没搜到达到赞数门槛的高赞内容。_\n")
    for i, pick in enumerate(data.top_voted, start=1):
        t = pick.item
        lines.append(f"### {i}. {t.title}")
        lines.append(_author_line_md(t, show_credibility=False))
        lines.append("")
        if t.content_text:
            lines.append(_full_text(t.content_text))
            lines.append("")
        if pick.note:
            lines.append(pick.note)
            lines.append("")
        lines.append(f"[🔗 阅读原文]({t.clean_url})")
        lines.append("")

    lines.append("## 三、搞笑精选（神回复 · 沙雕新闻 · 高赞段子）")
    lines.append("")
...
def render_html(data: DigestData) -> str:
    ...
    P.append(
        ...
        f'<li>全站高赞 <strong>{len(data.top_voted)}</strong> 条（不限话题，纯按赞数）</li>'
    ...
    # ---- 二、全站高赞（不限话题）----
    P.append(_section_label("全站高赞 · 不限话题"))
    P.append(f'<div style="font-size:12px;color:{_MUTE};margin:-4px 0 4px">排序：纯按赞数，谁赞高谁上（不限话题、不看作者，只取较近发布）</div>')
    for i, pick in enumerate(data.top_voted, start=1):
        t = pick.item
        P.append(_card_open())
        P.append(
            f'<div style="font-size:16px;font-weight:700;margin-bottom:2px">'
            f'<span style="color:{_BLUE}">{i}.</span> {_esc(t.title)}</div>'
        )
        P.append(_author_html(t, show_credibility=False))
        if t.content_text:
            P.append(
                f'<div style="font-size:14px;color:#374151;line-height:1.75">{_content_html(t.content_text)}</div>'
            )
        P.append(_note_box(pick.note or "", accent="#2ea36f"))
        P.append(_link_button(t.clean_url, "阅读原文"))
        P.append("</div>")
